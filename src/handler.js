...
const getAllBookHandler = (request, h) => {
  ...
  // cek apakah ada query yang dikirim
  const { name, reading, finished } = request.query

  //  cek query name
  if (request.query.name) {
    const bookByname = books
  ...
  if (request.query.reading) {
    if (reading === '1') {
      const bookByReading = books
        .filter((book) => book.reading === true)
        .map((book) => ({ id: book.id, name: book.name, publisher: book.publisher }))

      if (bookByReading !== undefined) {
        return {
          status: 'success',
          data: {
            books: bookByReading
  ...
  if (request.query.finished) {
    if (finished === '1') {
      const bookByFinished = books
        .filter((book) => book.finished === true)
        .map((book) => ({ id: book.id, name: book.name, publisher: book.publisher }))
      if (bookByFinished !== undefined) {
        return {
          status: 'success',
          data: {
            books: bookByFinished
          }
        }
      }
    } else {
